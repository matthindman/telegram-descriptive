...
CONTRACTS: Mapping[str, TableContract] = {
    ...
    "silver_language_labels": TableContract(
        name="silver_language_labels",
        grain="one language label per run and channel or message entity",
        primary_key=("lid_run_id", "entity_type", "entity_id"),
        required_columns=(
            c("lid_run_id", "string", False),
            c("entity_type", "string", False),
            c("entity_id", "string", False),
            ...
            c("language_margin", "numeric"),
            c("language_evidence_weight", "numeric"),
            c("language_segment_count", "integer"),
            c("label_source", "string"),
        ),
    ),
    ...
    "silver_topic_labels": TableContract(
        name="silver_topic_labels",
        grain="one topic label per taxonomy run and channel entity",
        primary_key=("topic_run_id", "entity_id"),
        required_columns=(
            c("topic_run_id", "string", False),
            c("taxonomy_version", "string", False),
            ...
            c("classification_method", "string"),
            c("evidence_ids", "array<string>"),
        ),
    ),
    ...
    "gold_post_ingestion_audit": TableContract(
        name="gold_post_ingestion_audit",
        grain="one selected or observed channel ingestion audit row",
        primary_key=("sample_version", "canonical_channel_id"),
        required_columns=(
            c("sample_version", "string", False),
            c("canonical_channel_id", "string", False),
            c("expected_in_sample", "boolean"),
            c("observed_message_count", "integer"),
            c("first_published_at", "timestamp"),
            c("last_published_at", "timestamp"),
            c("post_metrics_available", "boolean"),
            c("post_ingestion_status", "string"),
            c("audit_flags", "array<string>"),
        ),
    ),
    ...
    "gold_tail_parameters": TableContract(
        name="gold_tail_parameters",
        grain="one rank-tail parameter row per estimate version, metric, boundary, and model",
        primary_key=("estimate_version", "metric_name", "boundary_rank", "model"),
        required_columns=(
            c("estimate_version", "string", False),
            c("metric_name", "string", False),
            c("boundary_rank", "integer", False),
            c("model", "string", False),
            c("y0", "numeric"),
            c("alpha0", "numeric"),
            c("eta0", "numeric"),
            c("eta1", "numeric"),
            c("eta2", "numeric"),
            c("fitting_window", "integer"),
            c("observed_head_mass", "numeric"),
            c("tail_mass", "numeric"),
            c("total_mass", "numeric"),
            c("diagnostic_flags", "array<string>"),
        ),
    ),
    "gold_descriptive_summaries": TableContract(
        name="gold_descriptive_summaries",
        grain="one descriptive statistic per run, section, cohort, metric, and statistic",
        primary_key=("run_id", "section", "cohort", "metric", "statistic"),
        required_columns=(
            c("run_id", "string", False),
            c("section", "string", False),
            c("cohort", "string", False),
            c("metric", "string", False),
            c("statistic", "string", False),
            c("value", "numeric"),
            c("n", "integer"),
            c("notes", "string"),
        ),
    ),
    "gold_network_summaries": TableContract(
        name="gold_network_summaries",
        grain="one network statistic per run, edge type, node or cohort",
        primary_key=("run_id", "edge_type", "node_id", "statistic"),
        required_columns=(
            c("run_id", "string", False),
            c("edge_type", "string", False),
            c("node_id", "string", False),
            c("statistic", "string", False),
            c("value", "numeric"),
            c("rank", "integer"),
            c("notes", "string"),
    ...
    "gold_robustness_summaries": TableContract(
        name="gold_robustness_summaries",
        grain="one robustness or sensitivity check row",
        primary_key=("run_id", "check_name", "scenario", "metric"),
        required_columns=(
            c("run_id", "string", False),
            c("check_name", "string", False),
            c("scenario", "string", False),
            c("metric", "string", False),
            c("baseline_value", "numeric"),
            c("scenario_value", "numeric"),
            c("delta", "numeric"),
            c("notes", "string"),
        ),
    ),
    "gold_reporting_manifest": TableContract(
        name="gold_reporting_manifest",
        grain="one reporting/export artifact row",
        primary_key=("run_id", "artifact_name"),
        required_columns=(
            c("run_id", "string", False),
            c("artifact_name", "string", False),
            c("artifact_type", "string"),
            c("source_table", "string"),
            c("row_count", "integer"),
            c("created_at_utc", "timestamp"),
            c("notes", "string"),
        ),
    ),
